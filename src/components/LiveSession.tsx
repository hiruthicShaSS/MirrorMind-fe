...
export const LiveSession: React.FC<LiveSessionProps> = ({
  ...
  // No session: show Start session in a glassmorphism bar
  if (!session) {
    return (
      <div className="absolute bottom-0 left-0 right-0 z-20 flex justify-center p-4">
        <div className="w-full max-w-2xl rounded-2xl border border-white/20 bg-white/5 backdrop-blur-xl shadow-xl px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-gray-500">
                Start a session to use voice and chat
              </p>
            </div>
          </div>
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/20 border border-white/30 text-white font-mono font-bold uppercase text-xs tracking-wider hover:bg-white/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
